      ...
      <div className="relative bg-gradient-to-br from-slate-900/80 to-slate-950/60 h-72 flex items-center justify-center overflow-hidden">
        ...
        {(product.is_new || product.on_sale || product.is_best_seller) && (
          <div className="absolute top-3 right-3 flex flex-col gap-2">
            {product.is_new && (
              <span className="text-xs font-black px-3 py-1.5 rounded-full bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg shadow-sky-500/50">NEW</span>
            )}
            {product.on_sale && (
              <span className="text-xs font-black px-3 py-1.5 rounded-full bg-gradient-to-r from-red-500 to-rose-500 text-white shadow-lg shadow-red-500/50">SALE</span>
            )}
...
export default function ShopNowPage() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedBrand, selectedGender, searchQuery, sortBy]);
  ...
  const filteredItems = useMemo(() => {
    return items.filter((product) => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!name.includes(query) && !brand.includes(query) && !category.includes(query)) return false;
      }
      return true;
    });
  ...
  const sortedItems = useMemo(() => {
    ...
    switch (sortBy) {
      case "price-asc":  return list.sort((a, b) => Number(a.base_price || 0) - Number(b.base_price || 0));
      case "price-desc": return list.sort((a, b) => Number(b.base_price || 0) - Number(a.base_price || 0));
      case "name-asc":   return list.sort((a, b) => String(a.name || "").localeCompare(String(b.name || "")));
      case "name-desc":  return list.sort((a, b) => String(b.name || "").localeCompare(String(a.name || "")));
      case "newest":
        return list.sort((a, b) => {
          const da = Date.parse(a.created_at || a.updated_at || 0) || 0;
          const db = Date.parse(b.created_at || b.updated_at || 0) || 0;
          return db - da;
        });
      default: return list;
    }
  ...
  const paginatedItems = useMemo(() => {
    ...
    return sortedItems.slice(start, start + PRODUCTS_PER_PAGE);
      ...
      <div className="flex flex-col items-center gap-4 mt-12">
        <p className="text-sm text-slate-400">
          ...
          <span className="font-bold text-slate-200">
            {(currentPage - 1) * PRODUCTS_PER_PAGE + 1}–{Math.min(currentPage * PRODUCTS_PER_PAGE, sortedItems.length)}
          </span>{" "}
          of <span className="font-bold text-slate-200">{sortedItems.length}</span> products
        </p>
        <div className="flex items-center gap-2 flex-wrap justify-center">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
              currentPage === 1
                ? "bg-slate-900/30 border-2 border-slate-800/30 text-slate-600 cursor-not-allowed"
                : "bg-slate-900/60 border-2 border-slate-700 text-slate-300 hover:border-sky-500/50 hover:text-sky-400 hover:scale-105"
            }`}
          >
            ...
            page === "..." ? (
              ...
              <button
                ...
                className={`w-11 h-11 rounded-xl text-sm font-bold transition-all ${
                  currentPage === page
                    ? "bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg shadow-sky-500/40 scale-110 border-2 border-sky-400"
                    : "bg-slate-900/60 border-2 border-slate-700 text-slate-300 hover:border-sky-500/50 hover:text-sky-400 hover:scale-105"
                }`}
              >
                {page}
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
        ...
        <div className="mb-8 animate-fade-in animation-delay-100">
          <div className="relative max-w-2xl">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-300 transition-colors">
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center justify-between mb-4">
                <label className="text-xs font-black uppercase tracking-wider text-slate-400">Filter by Brand</label>
                {hasActiveFilters && (
                  <button onClick={clearFilters} className="text-xs font-bold text-sky-400 hover:text-sky-300 transition-colors">
                    Clear All Filters
                  </button>
                )}
              </div>
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setSelectedBrand("ALL")}
                  className={`px-5 py-3 rounded-xl font-bold text-sm transition-all ${selectedBrand === "ALL" ? "bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg shadow-sky-500/40 scale-105" : "bg-slate-800/60 text-slate-300 border-2 border-slate-700 hover:border-sky-500/50 hover:bg-slate-800 hover:scale-105"}`}
                >All Brands</button>
                {["Nike", "Jordan", "Adidas"].map((brand) => {
                  const hasBrand = availableBrands.includes(brand.toLowerCase());
                  return (
                    <button key={brand} onClick={() => setSelectedBrand(brand)} disabled={!hasBrand}
                    ...
                    >{brand}{!hasBrand && <span className="ml-2 text-xs">(0)</span>}</button>
                  );
                })}
                {availableBrands.filter((b) => !["nike", "jordan", "adidas"].includes(b)).map((brand) => (
                  ...
                  >{brand.charAt(0).toUpperCase() + brand.slice(1)}</button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-slate-400 mb-4">Filter by Gender</label>
                <div className="flex flex-wrap gap-3">
                  {["ALL", "M", "W"].map((g) => {
                    const label = g === "ALL" ? "All" : g === "M" ? "Men" : "Women";
                    return (
                      <button key={g} onClick={() => setSelectedGender(g)}
                        className={`px-5 py-3 rounded-xl font-bold text-sm transition-all ${selectedGender === g ? "bg-gradient-to-r from-sky-500 to-cyan-500 text-white shadow-lg shadow-sky-500/40 scale-105" : "bg-slate-800/60 text-slate-300 border-2 border-slate-700 hover:border-sky-500/50 hover:bg-slate-800 hover:scale-105"}`}
                      >{label}</button>
                    );
                  })}
                </div>
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-slate-400 mb-4">Sort By</label>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 px-6 rounded-2xl bg-slate-900/40 border-2 border-slate-800/50">
            ...
            <button onClick={clearFilters} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-sky-500 to-cyan-500 text-white font-bold hover:from-sky-400 hover:to-cyan-400 shadow-lg shadow-sky-500/30 transition-all">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <>
            {/* Results Count */}
            <div className="flex items-center justify-between mb-6" ref={gridRef}>
              <div className="text-sm text-slate-400">
                Showing{" "}
                <span className="text-sky-400 font-bold text-base">
                  {(currentPage - 1) * PRODUCTS_PER_PAGE + 1}–{Math.min(currentPage * PRODUCTS_PER_PAGE, sortedItems.length)}
                </span>{" "}
                of <span className="text-slate-300 font-bold">{sortedItems.length}</span> products
                {sortedItems.length < items.length && (
                  <span className="text-slate-500"> (filtered from {items.length})</span>
                )}
              </div>
              {hasActiveFilters && (
                <button onClick={clearFilters} className="text-sm font-bold text-sky-400 hover:text-sky-300 transition-colors flex items-center gap-2">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                  Clear Filters
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
              {paginatedItems.map((p, idx) => (
                <div key={p.id} className="animate-fade-in" style={{ animationDelay: `${idx * 50}ms` }}>
